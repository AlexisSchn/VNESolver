...
function solve_subgraph_decomposition(instance::Instance)
    ...
    while keep_on && time_overall < time_max && iter < iter_max
        ...
        for v_subgraph in v_subgraphs
            ...
            #=
            if !isnothing(submapping) && column_already_there(columns, submapping, v_subgraph)
                println("Already there! $reduced_cost")
            end
            =#

            #println("red cost: $reduced_cost, but real red cost: $(compute_reduced_costs(submapping, v_subgraph, dual_costs, instance))")
        ...
        time_overall = time() - time_beginning
        @printf("Iter %-3d; RMP value: %8.3f;   LG bound: %6.3f,   Pricer: %-8s;   New columns %-2d;   Total columns %-4d;   Aver. red. %5.3f;   Time %5.3f\n", 
            iter, rmp_value, lg_bound, pricer, nb_new_cols, nb_total_columns, average_reduced_cost, time_overall)

    end
    
    println("Finished! somevalue in $(time()-time_beginning), with $(time_pricer) for pricer and $(time_rmp) for RMP.")
