...
function solve_flow_formulation(instance::InstanceVNE)
...
function set_up_formulation(model, instance)
    ...
    @variable(model, y[edges(v_g), edges(sdir)], binary=true);
    ...
    for v_edge in edges(v_g), s_edge in edges(s_g)
        if ve_dem[src(v_edge), dst(v_edge)] > se_cap[src(s_edge), dst(s_edge)]
            ...
            fix(y[v_edge, reverse_edge(s_edge)], 0; force=true)
        end
    end
    ...
    @constraint(model, [s_edge in edges(s_g)], 
        sum(
            ve_dem[src(v_edge), dst(v_edge)] * ( y[v_edge, s_edge] + y[v_edge, reverse_edge(s_edge)]) for v_edge in edges(v_g)) 
    ...
    @constraint(model, [s_node in vertices(s_g), v_edge in edges(v_g)],
        ...
        == sum(y[v_edge, Edge(s_node, s_dst)] for s_dst in outneighbors(sdir, s_node))
            - sum(y[v_edge, Edge(s_src, s_node)] for s_src in inneighbors(sdir, s_node))
    )
...
function add_valid_inequalities(model, instance)
    ...
    s_g, sdir = instance.s_network.graph, instance.s_network.directed_graph
    x, y = model[:x], model[:y]
